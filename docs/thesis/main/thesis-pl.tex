...
Standardowy mechanizm uwagi w~architekturze Transformer charakteryzuje się 
kwadratową złożonością obliczeniową $O(N^2)$, co stanowi istotne ograniczenie 
przy przetwarzaniu długich sekwencji tekstowych. Celem pracy jest implementacja 
enkodera typu BERT umożliwiającego wymianę modułu uwagi oraz porównanie trzech 
mechanizmów: standardowego SDPA, LSH (Reformer) oraz FAVOR+ (Performer), pod 
kątem zarówno jakości klasyfikacji, jak i~czasu uczenia oraz wymaganych zasobów. Zaprojektowano modułowy 
system obejmujący architekturę transformera, tokenizer, pętlę treningową 
z~obsługą wstępnego trenowania i~dostrajania, deklaratywną konfigurację eksperymentów oraz integrację z~platformą 
Weights~\&~Biases. Dla mechanizmu LSH zaproponowano autorskie podejście do sposobu maskowania przy obliczaniu uwagi. Przeprowadzono 
eksperymenty na trzech zbiorach danych (IMDb, Hyperpartisan, arXiv) o~długościach 
sekwencji 512--16384 tokenów, stosując trzyetapowy proces uczenia: wstępne 
trenowanie z~wykorzystaniem maskowanego modelowania języka, adaptację domenową 
oraz dostrajanie do zadania klasyfikacji. Dla sekwencji długości $16384$ tokenów 
przybliżone mechanizmy uwagi (w~najszybszych konfiguracjach) umożliwiły redukcję czasu uczenia o~ponad $50\%$ 
względem SDPA, przy niewielkim spadku metryki 
F1-macro.

\vspace{1em}

\noindent\textbf{Słowa kluczowe:} transformer, mechanizm uwagi, BERT, Performer, Reformer, klasyfikacja tekstu, przetwarzanie języka naturalnego
...
The standard attention mechanism in the Transformer architecture exhibits 
quadratic computational complexity $O(N^2)$, which poses a~significant 
limitation when processing long text sequences. The aim of this thesis is 
to implement a~BERT-type encoder with interchangeable attention modules 
and to compare three mechanisms: standard SDPA, LSH (Reformer), and 
FAVOR+ (Performer), in terms of classification quality, training time, 
and resource requirements. A~modular system was designed, comprising the 
transformer architecture, a~tokenizer, a~training loop supporting pretraining 
and fine-tuning, declarative experiment configuration, and integration with 
the Weights~\&~Biases platform. For the LSH mechanism, a novel approach to attention masking was proposed. Experiments were conducted on three datasets 
(IMDb, Hyperpartisan, arXiv) with sequence lengths ranging from 512 to 16384 
tokens, employing a~three-stage training process: pretraining using masked 
language modeling, domain adaptation, and fine-tuning for the classification 
task. For sequences of 16384 tokens, approximate attention mechanisms 
(in their fastest configurations) enabled a~reduction in training time of over 50\% 
compared to SDPA, with a~slight decrease in the F1-macro metric.

\vspace{1em}
\noindent\textbf{Keywords:} transformer, attention mechanism, BERT, Performer, Reformer, text classification, NLP
\end{abstract}
}
...
% ------------------- 4. podział pracy ---------------------
\section*{Podział pracy autorów}
\vspace{3pt}

\begin{table}[!h]
\centering
% \caption{Wkład autorów pracy dyplomowej}
...
\hline
\textbf{Rola} &
\textbf{Bartłomiej Borycki \newline nr albumu: 327265} &
\textbf{Michał Iwaniuk \newline nr albumu: 327278} \\
\hline

Przygotowanie tekstu pracy -- wersja początkowa &
Rozdziały: 1, 4, 5, 6 &
Rozdziały: 2, 3, 5, 6 \\
\hline

Przygotowanie tekstu pracy -- przegląd i redakcja &
...
\begin{itemize}[leftmargin=*, nosep]
    \item Mechanizm uwagi FAVOR+
    \item Klasa Transformer
    \item Skrypty treningowe
    \item Generator eksperymentów
\end{itemize} &
\begin{itemize}[leftmargin=*, nosep]
    \item Mechanizm uwagi SDPA
    \item Mechanizm uwagi LSH
    \item Bloki enkodera
    \item Testy jednostkowe
\end{itemize} \\
\hline

Kluczowe rozwiązania pracy:\newline
konceptualizacja, badania oraz metodologia &
Projekt architektury systemu, trening modeli, optymalizacja procesu
treningu i dobór hiperparametrów, implementacja rozwiązań badawczych
(mechanizm FAVOR+) &
Projekt architektury systemu, trening modeli, opracowanie metodyki
eksperymentów, implementacja rozwiązań badawczych
(mechanizm LSH) \\
\hline

Analiza formalna i walidacja &
Analiza wyników eksperymentów, porównanie mechanizmów uwagi &
Analiza wyników eksperymentów, walidacja testami jednostkowymi,
analiza eksploracyjna danych \\
\hline

Przygotowanie danych &
Przygotowanie zbiorów IMDb i Hyperpartisan &
Przygotowanie zbiorów Wikipedia i ArXiv \\
\hline
...
% OBOWIĄZKOWA DEKLARACJA POCZĄTEK (należy wybrać odpowiednie)
% Do wykorzystania w pracach inżynierskich; do usunięcia w pracach magisterskich
% {\em The content below SHOULD BE REMOVED IN MSc thesis}

\section*{Deklaracja użycia narzędzi AI}
\vspace{3pt}

\normalsize
Niniejszym oświadczamy, że:
\begin{enumerate}
    \item Nie używaliśmy narzędzi informatycznych do generowania treści niniejszej pracy.
    \item Używaliśmy narzędzi informatycznych do generowania kodu programów stworzonych na potrzeby niniejszej pracy.
    \item Bierzemy pełną odpowiedzialność za zawartość niniejszej pracy, która składa się zarówno z jej tekstu, jak i stworzonego na jej potrzeby oprogramowania.
\end{enumerate}
% OBOWIĄZKOWA DEKLARACJA KONIEC

% Przez narzędzia informatyczne rozumiane są narzędzia generujące tekst, treści multimedialne i kod oprogramowania, w tym między innymi narzędzia bazujące na modelach językowych, takie jak ChatGPT, Gemini, Copilot itp. W stwierdzeniach 1 i 2 proszę zaznaczyć opcję odpowiadającą sposobowi przygotowania pracy dyplomowej.

% TABELA TYLKO JEŚLI TRZEBA COŚ DO NIEJ WPISAĆ
\begin{table}[h]
\centering
\begin{tabular}{|p{0.5\textwidth}|p{0.5\textwidth}|}
\hline
\textbf{Zakres użycia narzędzi informatycznych do generowania tekstu pracy} & \textbf{Zakres użycia narzędzi informatycznych do generowania kodu} \\ \hline
   &  Generowanie opisów funkcji (ang. \emph{docstring}) oraz komentarzy, generowanie testów jednostkowych kodu, funkcje pomocnicze do wczytywania i obsługi plików YAML i CSV  \\ \hline
\end{tabular}
\end{table}
...
\subsubsection{Modele przestrzeni stanów}

\textbf{Mamba} (2024)~\cite{gu2024mamba} -- architektura oparta na~selektywnych modelach przestrzeni stanów (ang.\ \emph{Selective State Space Models}, SSM), stanowiąca alternatywę dla~mechanizmu uwagi. W~przeciwieństwie do~transformerów, Mamba przetwarza sekwencję w~czasie liniowym~$O(N)$ zarówno podczas treningu, jak i~inferencji, dynamicznie dostosowując parametry modelu w~zależności od~wejścia. Architektura ta osiąga konkurencyjne wyniki przy znacznie niższych wymaganiach obliczeniowych dla~bardzo długich sekwencji.
...
\textbf{Mistral 7B} (2023)~\cite{jiang2023mistral} -- model językowy, który spopularyzował efektywne połączenie uwagi z~oknem przesuwnym (ang.\ \emph{Sliding Window Attention}, SWA) oraz \emph{Grouped-Query Attention} (GQA). Każdy token zwraca uwagę jedynie na~ograniczoną liczbę poprzedzających tokenów (np.\ 4096), co redukuje złożoność pamięciową do~$O(N \cdot w)$, gdzie $w$~to rozmiar okna. Dzięki propagacji informacji przez kolejne warstwy model efektywnie agreguje kontekst z~całej sekwencji. Podejście to zostało zaadoptowane w~wielu późniejszych modelach, m.in.\ Gemma~2~\cite{gemmateam2024gemma2} i~Phi-3~\cite{abdin2024phi3}.
...
\textbf{FlashAttention-2} (2024)~\cite{dao2024flashattention} oraz \textbf{FlashAttention-3} (2024)~\cite{shah2024flashattention} -- kolejne wersje algorytmów IO-aware dla dokładnej uwagi, kładące nacisk na lepszy podział pracy i wykorzystanie nowoczesnych GPU (szczególnie H100), co daje istotne przyspieszenie bez aproksymacji.
...
\subsubsection{Prawa skalowania}

Równolegle z rozwojem architektur prowadzone są badania nad prawami skalowania (ang.\ \emph{scaling laws}),
które empirycznie opisują zależności między wielkością modelu, ilością danych treningowych i budżetem obliczeniowym
a uzyskiwaną jakością \cite{kaplan2020scalinglawsneurallanguage,hoffmann2022training}.
Należy jednak podkreślić, że większość tych wyników dotyczy modeli średnich i dużych (miliardy parametrów),
a w ustawieniach niskozasobowych obserwuje się istotne odchylenia od klasycznych zależności --
optymalna relacja między liczbą parametrów a liczbą tokenów może być przesunięta w stronę większej ilości danych, a prognozy dla małych modeli oparte na prawach wyznaczonych dla dużych modeli
mogą być nietrafne~\cite{urbizu-etal-2023-scaling}.
W niniejszej pracy nie ekstrapolujemy praw skalowania wprost na mały model (rzędu $30M$ parametrów),
lecz wykorzystujemy ich główną, jakościową implikację -- przy ograniczonym budżecie obliczeniowym
istotne jest możliwie efektywne wykorzystanie dostępnych danych. Motywuje to poszukiwanie bardziej efektywnych mechanizmów uwagi, które redukują koszt obliczeniowy
i umożliwiają dłuższe trenowanie (lub użycie dłuższych sekwencji / większej liczby tokenów) przy stałym budżecie.
...
\begin{itemize}
  \item \textbf{Wejście:} stokenizowane korpusy tekstowe, plik YAML z~konfiguracją, tryb \texttt{pretrain} lub \texttt{finetune}.
...
% Wkład poszczególnych autorów jest przedstawiony w~tab.~\ref{tab:podzial-prac}.
...
% UWAGA: Wymaga pakietu enumitem dla list wewnątrz tabeli:
% \usepackage{enumitem}
...
Implementacja obsługuje parametr \texttt{tie\_mlm\_weights}. Gdy jest on ustawiony na~\texttt{True}, wagi warstwy wyjściowej (dekodującej) są współdzielone z~wagami macierzy osadzeń wejściowych, co jest standardową praktyką w~modelach typu BERT~\cite{devlin2019bert}.
...
Klasa \texttt{FAVORAttention} implementuje mechanizm uwagi o~złożoności czasowej oraz pamięciowej~$O(N)$~\cite{choromanski2021rethinking}. Zamiast obliczać pełną macierz uwagi $N \times N$, model aproksymuje funkcję Softmax, wykorzystując mapowania cech $\phi(\cdot)$ (szczegóły w~sek.~\ref{sec:favor}).
...
Klasa \texttt{LSHAttention} implementuje uwagę o~złożoności czasowej $O(N \log N)$ oraz pamięciowej $O(N)$~\cite{Kitaev2020Reformer:}. Mechanizm ten zakłada, że tokeny powinny zwracać uwagę głównie na tokeny do nich podobne (znajdujące się w~tym samym tzw. kubełku haszującym).

Kluczowe elementy implementacji (szczegóły w sek.~\ref{sec:lsh}):
\begin{itemize}
    \item \textbf{Współdzielone Q~i~K:} Zgodnie z~architekturą Reformera~\cite{Kitaev2020Reformer:}, projekcje zapytań i~kluczy są tożsame ($Q=K$), co jest wymagane do~poprawnego działania LSH.
...
Standardowa strategia dla modeli typu BERT~\cite{devlin2019bert}. Jako reprezentację całej sekwencji przyjmuje się stan ukryty pierwszego tokenu specjalnego (zwyczajowo \texttt{[CLS]}).
\begin{equation}
...
\begin{itemize}
    \item \textbf{Styl BERT~\cite{devlin2019bert}:} Składa się z~warstwy gęstej zachowującej wymiarowość, funkcji aktywacji Tanh, a~następnie warstwy wyjściowej:
    \begin{equation}
    y = \text{Linear}(\text{Dropout}(\text{Tanh}(\text{Linear}(x)))).
    \end{equation}
    \item \textbf{Styl RoBERTa~\cite{liu2019roberta}:} Charakteryzuje się dodatkowym Dropoutem na wejściu:
    \begin{equation}
...
Głowica służąca do pretreningu w zadaniu MLM. Przyjmuje ona sekwencję stanów ukrytych $(B, N, D)$ i zwraca predykcje dla każdego tokenu w słowniku $(B, N, V)$. Struktura głowicy jest zgodna ze standardem BERT~\cite{devlin2019bert} i obejmuje:
\begin{enumerate}
...
\subsection{Logika pętli treningowej}\label{sec:training-loop}
...
\begin{itemize}
    \item Kontynuacja przerwanego treningu: Ustaw \texttt{false}, aby wczytać pełny stan (model, optymalizator, scheduler, skaler).
    \item Transfer learning / TAPT: Ustaw \texttt{true}, aby wczytać wyłącznie wagi modelu.
\end{itemize}
...
W~Transformerze~\cite{vaswani2017attention} podstawową operacją jest wielogłowowa samouwaga (\emph{Multi-Head Self Attention}) realizowana przez~SDPA (\emph{Scaled Dot-Product Attention}). Dzięki temu mechanizmowi każda pozycja sekwencji buduje kontekstową reprezentację, która następnie jest wykorzystywana w~kolejnych warstwach modelu. 
...
może wykorzystywać zoptymalizowane algorytmy, takie jak FlashAttention.
...
\emph{Locality-Sensitive Hashing attention} (zaproponowana w~\cite{Kitaev2020Reformer:}) jest przybliżeniem pełnej uwagi, które redukuje liczbę porównań przez~ograniczenie uwagi do~elementów podobnych -- identyfikowanych za~pomocą haszowania.
...
\textit{Uwaga:} Dla przejrzystości pomijamy skalowanie przez $\sqrt{d_k}$ (tutaj i w dalszej części), w implementacji uwzględniamy to skalowanie.
...
Wpływ tej modyfikacji na~trening modelu znajduje się w~sek.~\ref{sec:lsh-mask-experiment}. W~opisanych w~dalszej części pracy eksperymentach (sek.~\ref{sec:experiments}) będziemy korzystać z~tej zmodyfikowanej wersji LSH (brak maski).
...
na~same wagi uwagi. Metoda FAVOR+ (\emph{Fast Attention Via positive Orthogonal Random features} zaproponowana w~\cite{choromanski2021rethinking})
...
% Technika aproksymacji kerneli za~pomocą losowych cech została wprowadzona 
% w~\cite{rahimi2007random}, a~następnie rozszerzona o~cechy ortogonalne 
% w~\cite{yu2016orthogonal}.
...
W~naszej pracy używamy modyfikacji estymatora $\exp$ zaproponowanej i opisanej teoretycznie w~\cite{choromanski2021rethinking}, lecz nieprzetestowanej eksperymentalnie przez autorów:
...
Następnie:
\begin{itemize}
  \item $450\,000$ artykułów stokenizowano do maksymalnej długości $128$ tokenów\footnote{Dane zostały stokenizowane z wykorzystaniem \texttt{WordPieceTokenizerWrapper} (patrz sek.~\ref{sec:tokenizer}).\label{fn:data-tokenizer}},
  \item pozostałe $150\,000$ artykułów stokenizowano do maksymalnej długości $512$ tokenów\textsuperscript{\ref{fn:data-tokenizer}}.
\end{itemize}
...
\]
Dane stokenizowano do maksymalnej długości $512$ tokenów\textsuperscript{\ref{fn:data-tokenizer}}.
...
\]
Dane stokenizowano do maksymalnej długości $4\,096$ tokenów\textsuperscript{\ref{fn:data-tokenizer}}.
...
\]
Dane stokenizowano do maksymalnej długości $16\,384$ tokenów\textsuperscript{\ref{fn:data-tokenizer}}.
...
\begin{table}[t!]
\centering
\caption{Statystyki długości tekstu (liczba tokenów) po tokenizacji}
...
W~celu przeprowadzenia pełnego eksperymentu porównawczego zaprojektowano wieloetapowy proces uczenia, który rozpoczyna się od pretreningu na korpusie ogólnym (Wikipedia) z~wykorzystaniem zadania modelowania języka z~maskowaniem (ang.\ \emph{Masked Language Modeling}, MLM) w~celu nauczenia modelu ogólnych struktur językowych i~semantyki, co kończy się zapisem stanu modelu (ang.\ \emph{checkpoint}) w~odpowiednim eksperymencie. Następnie realizowana jest adaptacja domenowa (ang.\ \emph{Task-Adaptive Pretraining}, TAPT~\cite{gururangan-etal-2020-dont}), polegająca na kontynuacji uczenia MLM na nieetykietowanych danych z~domeny docelowej -- w~niniejszej pracy wykorzystano do tego celu te same zbiory, które służą do późniejszej klasyfikacji (IMDb, Hyperpartisan, ArXiv), przeprowadzając proces TAPT każdorazowo wyłącznie na zbiorze docelowym, startując z~modelu bazowego i~zapisując stan modelu po~eksperymencie TAPT. Ostatnim etapem jest końcowe dostrajanie (ang.\ \emph{fine-tuning}), w~którym model jest trenowany w~sposób nadzorowany na konkretnym, etykietowanym zbiorze danych, startując z~odpowiedniego zapisanego stanu TAPT, co pozwala na uzyskanie końcowego modelu \texttt{model.pt} oraz jego najlepszej wersji \texttt{best\_model.pt}, wyłonionej na podstawie wyników F1-macro na zbiorze walidacyjnym. Po każdym z~etapów następuje transfer wag z~odpowiedniego punktu zapisanego stanu, podczas gdy pozostałe parametry, takie jak stan optymalizatora, harmonogram współczynnika uczenia (ang.\ \emph{learning rate scheduler}) oraz skaler gradientów (ang.\ \emph{gradient scaler}), są resetowane. Ze względu na wysokie koszty obliczeniowe, każdą konfigurację trenowano jednokrotnie. Przy interpretacji wyników należy uwzględnić, że różnice rzędu dziesiątych części punktu procentowego mogą wynikać z~losowości procesu uczenia.
...
We wszystkich eksperymentach wykorzystano architekturę $\text{BERT}_{\text{SMALL}}$, wprowadzoną w~artykule \textcite{turc2019well}.
Konfiguracja: $l^t = 4$, $d^h = 512$, $d^f = 2048$, $h = 8$, $d^{q|k|v} = 64$.
...
Należy zaznaczyć, że ostatnia warstwa liniowa w~głowicy MLM współdzieli wagi z~warstwą osadzeń. Całkowitą liczbę parametrów modelu wyrażamy wzorem:
\begin{equation}
...
\midrule
FAVOR+
    & Liczba losowych cech ($N_f$) & $\{0.125, 0.25, 0.5, 1.0\} \times d^h$ \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Środowisko eksperymentalne i zużyte zasoby}

Wszystkie eksperymenty przeprowadzono w środowisku Google Colab z wykorzystaniem karty NVIDIA A100 ($40$ GB pamięci VRAM). Pełny cykl eksperymentów obejmujący wszystkie etapy (\ref{sec:stage1}, \ref{sec:stage2}, \ref{sec:stage3}),  zajął łącznie $169.48$ godzin czasu GPU. Przekłada się to na zużycie około $910$ jednostek obliczeniowych (ang. \emph{compute units}) Google Colab, przy koszcie $5.37$ jednostek obliczeniowych za godzinę.
...
\section{Etap 2: Adaptacja domenowa}\label{sec:stage2}
...
\section{Etap 3: Dostrajanie}\label{sec:stage3}
...
\midrule
\multicolumn{4}{l}{\textit{Agregacja Mean}} \\
...
\begin{table}[t!]
\centering
\caption{Porównanie mechanizmów uwagi przy dostrajaniu: VRAM, czas na epokę i F1 macro [\%] dla różnych zbiorów.}
...
\begin{itemize}
    ...
    \item \textbf{FAVOR+} uzyskał najsłabsze wyniki F1 na wszystkich zbiorach danych. Na IMDb i~ArXiv strata względem SDPA wynosi odpowiednio $1.1$--$1.9$~pp oraz $1.9$--$2.8$~pp. Szczególnie słabe wyniki FAVOR+ osiągnął na zbiorze Hyperpartisan -- strata względem SDPA sięga $5.3$--$11.1$~pp. Jednocześnie FAVOR+ oferuje przewagę czasową dla długich sekwencji (ArXiv: $22$--$52$~min vs $55$~min dla SDPA).
...
W~niniejszej sekcji przedstawiono zbiorczą analizę wyników ze wszystkich etapów eksperymentu. Tabele~\ref{tab:vram-comparison} i~\ref{tab:time-comparison} zestawiają względne różnice w~zużyciu pamięci VRAM oraz czasie treningu dla mechanizmów przybliżonych względem SDPA. Różnice procentowe w~czasie treningu obliczono na podstawie całkowitego czasu obu etapów (suma: liczba epok TAPT $\times$ czas epoki TAPT + liczba epok dostrajania $\times$ czas epoki dostrajania), a~nie pojedynczych epok. Tabele~\ref{tab:attention-f1-vs-baseline}~i~\ref{tab:attention-f1-vs-sdpa} podsumowują wyniki klasyfikacji F1-macro w~odniesieniu do baseline'u oraz SDPA.

Aby zidentyfikować konfiguracje oferujące najlepszy kompromis między jakością a~kosztem obliczeniowym, przeprowadziliśmy analizę Pareto. Konfiguracja należy do frontu Pareto, jeśli żadna inna konfiguracja nie jest od niej jednocześnie lepsza pod względem wszystkich trzech kryteriów: F1-macro, czasu treningu oraz zużycia VRAM. Analizę przeprowadzono dla etapów TAPT i~dostrajania łącznie, osobno dla każdego zbioru danych. Wyniki przedstawiono w~tab.~\ref{tab:pareto} Ze względu na brak replikacji eksperymentów, analiza Pareto ma charakter orientacyjny -- konfiguracje o~zbliżonych wartościach F1-macro (różnice $<1$~pp) mogłyby zmienić pozycję względem frontu przy powtórzeniu z~inną inicjalizacją.

\begin{table}[t!]
\centering
\caption{Analiza Pareto dla etapów TAPT + dostrajanie. Symbol \checkmark{} oznacza przynależność do frontu Pareto (optymalizacja: max F1, min czas, min VRAM).}
\smallskip
\label{tab:pareto}
\small
\begin{tabular}{@{}lccc@{}}
...
\begin{table}[t!]
\centering
\caption{Porównanie maksymalnego zużycia pamięci VRAM: pretrening na Wikipedii oraz TAPT+dostrajanie na zbiorach docelowych. Wartości pokazują różnicę procentową względem SDPA.}
\label{tab:vram-comparison}
\smallskip
\begin{tabular}{@{}lcccc@{}}
\toprule
& \textbf{pretrening} & \multicolumn{3}{c}{\textbf{TAPT + dostrajanie}} \\
\cmidrule(lr){2-2}
\cmidrule(lr){3-5}
...
\begin{table}[t!]
\centering
\caption{Porównanie czasu treningu: pretrening na Wikipedii oraz całkowity czas etapów TAPT~i dostrajanie  na zbiorach docelowych. Wartości pokazują różnicę procentową względem SDPA.}
...
\begin{table}[t!]
\centering
\caption{F1 macro [\%] - porównanie mechanizmów uwagi z baseline TF-IDF+LR. Dla TF-IDF+LR podano wartości bezwzględne, dla pozostałych modeli podano różnicę w punktach procentowych (pp) względem TF-IDF+LR.}
\label{tab:attention-f1-vs-baseline}
\smallskip
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Model} & \textbf{IMDb} & \textbf{Hyper.} & \textbf{Arxiv} \\
...
\textit{TF-IDF + LR} & 89.50 & 42.23 & 83.62 \\
\midrule
\textit{SDPA} & +3.4 & +22.3 & +4.8 \\
\midrule
\multicolumn{4}{l}{\textit{LSH}} \\
$N_h{=}2$, $C{=}64$ & +3.3 & +23.1 & +3.3 \\
$N_h{=}2$, $C{=}128$ & +3.2 & +19.0 & +3.7 \\
$N_h{=}4$, $C{=}64$ & +3.0 & +20.2 & +3.1 \\
$N_h{=}4$, $C{=}128$ & +3.5 & +13.4 & +3.8 \\
\midrule
\multicolumn{4}{l}{\textit{FAVOR+}} \\
$N_f{=}0.125$ & +1.7 & +11.2 & +2.4 \\
$N_f{=}0.25$ & +1.5 & +14.8 & +2.9 \\
$N_f{=}0.5$ & +2.1 & +17.1 & +2.9 \\
$N_f{=}1.0$ & +2.2 & +12.8 & +1.9 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t!]
\centering
\caption{F1 macro [\%] - porównanie mechanizmów uwagi z SDPA. Dla SDPA podano wartości bezwzględne, dla LSH i FAVOR+ podano różnicę w punktach procentowych (pp) względem SDPA.}
\label{tab:attention-f1-vs-sdpa}
\smallskip
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Model} & \textbf{IMDb} & \textbf{Hyper.} & \textbf{Arxiv} \\
\midrule
\textit{SDPA} & 92.86 & 64.55 & 88.38 \\
\midrule
\multicolumn{4}{l}{\textit{LSH}} \\
$N_h{=}2$, $C{=}64$ & -0.1 & +0.8 & -1.5 \\
$N_h{=}2$, $C{=}128$ & -0.2 & -3.3 & -1.1 \\
$N_h{=}4$, $C{=}64$ & -0.4 & -2.1 & -1.7 \\
$N_h{=}4$, $C{=}128$ & +0.1 & -8.9 & -1.0 \\
\midrule
\multicolumn{4}{l}{\textit{FAVOR+}} \\
$N_f{=}0.125$ & -1.7 & -11.1 & -2.4 \\
$N_f{=}0.25$ & -1.9 & -7.6 & -1.9 \\
$N_f{=}0.5$ & -1.2 & -5.2 & -1.9 \\
$N_f{=}1.0$ & -1.1 & -9.5 & -2.8 \\
\bottomrule
\end{tabular}
...
\section{Analiza jakościowa błędów klasyfikacji na zbiorze Hyperpartisan}

W celu głębszego zrozumienia przyczyn znacznie gorszych wyników mechanizmu  FAVOR+ względem standardowej uwagi SDPA na zbiorze Hyperpartisan, przeprowadzono analizę błędów klasyfikacji. Analizę przeprowadzono dla konfiguracji FAVOR+ z parametrem $N_f = 0.125$, która dała najgorsze wyniki w eksperymentach.

\subsection{Charakterystyka błędów}

Analiza macierzy pomyłek na losowej próbie $100$ przykładów ze zbioru testowego ujawniła istotną różnicę w charakterze błędów popełnianych przez oba modele. Próba zawierała $54$ przypadki klasy \textit{neutral} (klasa $0$) oraz $46$ przypadków klasy \textit{partisan} (klasa $1$). Wyniki przedstawiono w tabeli~\ref{tab:confusion_comparison_qual}.

\begin{table}[H]
\centering
\caption{Macierze pomyłek dla FAVOR+ ($N_f=0.125$) i SDPA na losowej próbie $100$ przykładów ze zbioru testowego}
\label{tab:confusion_comparison_qual}
\smallskip

\begin{tabular}{lcc}
\toprule
\multicolumn{3}{c}{\textbf{FAVOR+ ($N_f=0.125$)}} \\
\midrule
& \multicolumn{2}{c}{Prawdziwa etykieta} \\
\cmidrule(lr){2-3}
Predykcja & Neutral & Partisan \\
\midrule
Neutral & 19 & 8 \\
Partisan & 35 & 38 \\
\bottomrule
...
Wśród 21 przypadków, w których SDPA klasyfikowała poprawnie, a FAVOR+ błędnie, aż~$20 (95.2\%)$ stanowiły błędy typu \textit{false positive}. Oznacza to, że FAVOR+ wykazuje wyraźną tendencję do nadmiernego przypisywania etykiety \textit{partisan} tekstom neutralnym, podczas gdy SDPA zachowuje większą ostrożność w tej klasie.

\subsection{Analiza przykładów błędnych klasyfikacji}

Aby zidentyfikować potencjalne wzorce prowadzące do błędów FAVOR+, wybrano ze zbioru testowego $20$ przykładów klasy \textit{neutral}, w których FAVOR+ klasyfikował błędnie (błędy typu \textit{false positive}), podczas gdy SDPA klasyfikował poprawnie. Pełną listę przykładów wraz z początkowymi fragmentami tekstów przedstawiono w tabeli~\ref{tab:favor-fp-sdpa-correct}.


\paragraph{Długość sekwencji}
Spośród $20$ analizowanych przykładów, aż $5$ miało maksymalną długość $4096$ tokenów. Może to wynikać z różnic rozkładów długości w zbiorach (rys.~\ref{fig:train_lengths} i~\ref{fig:test_lengths}) oraz odwrócenia zależności długość–etykieta: w treningu odsetek dokumentów o długości $4096$ jest większy dla klasy $1$ (\textit{partisan}: $12.07\%$) niż dla klasy $0$ (\textit{neutral}: $5.97\%$), natomiast w teście sytuacja jest odwrotna (klasa $0$: $21.72\%$, klasa $1$: $6.58\%$). FAVOR+ dla bardzo długich sekwencji może tracić zdolność utrzymania kontekstu.

\begin{figure}[b!]
\centering
\includegraphics[width=0.85\textwidth]{static/train_lengths.pdf}
\caption{Rozkład długości sekwencji w zbiorze treningowym Hyperpartisan według etykiety}
\label{fig:train_lengths}
\end{figure}

\begin{figure}[t!]
\centering
\includegraphics[width=0.85\textwidth]{static/test_lengths.pdf}
\caption{Rozkład długości sekwencji w zbiorze testowym Hyperpartisan według etykiety}
\label{fig:test_lengths}
\end{figure}
\paragraph{Wzorce prefiksowe}
Istnieje korelację błędów z obecnością specyficznych wzorców leksykalnych na początku dokumentów. W szczególności:
\begin{itemize}
    \item $9$ z $20$ przykładów ($45\%$) rozpoczynało się od frazy \texttt{more on:} lub \texttt{for more on:},
    \item $10$ z $20$ przykładów $(50\%)$ zawierało substring \texttt{more on} w pierwszych $64$ tokenach.
\end{itemize}

Frazy te stanowią artefakt redakcyjny charakterystyczny dla publikacji pochodzących z określonych źródeł.
\begin{itemize}
    \item W zbiorze treningowym prefiks \texttt{more on} nie występuje w ogóle ($0\%$ w obu klasach),
    \item W zbiorze testowym występuje jako prefiks u 39,48\% dokumentów klasy 0 (\textit{neutral}),
    % \item Analiza występowania bigramu \texttt{more on} gdziekolwiek w dokumencie potwierdza ten wzorzec: train -- 2,22\% (label 0) vs test -- 54,12\% (label 0).
\end{itemize}

Ten wzorzec sugeruje, że FAVOR+ ma trudności z~rozpoznawaniem nowych, nieznanych z~treningu wzorców tekstowych (takich jak \texttt{more on:}). Model może mylnie traktować te artefakty formatowania jako wskaźnik tendencyjności tekstu.

\paragraph{Powiązanie długości i prefiksów}
Warto zauważyć, że oba zaobserwowane wzorce -- długie sekwencje oraz obecność artefaktów \texttt{more on} -- często występują jednocześnie. Spośród $5$ przykładów o maksymalnej długości $4096$ tokenów, $3$ rozpoczynają się od \texttt{more on}. Może to wskazywać na kumulatywny efekt: aproksymacja w FAVOR+ może być problematyczna gdy długi kontekst łączy się z nietypowymi wzorcami.

\begin{table}[h]
\centering
\caption{Porównanie liczby zmian w wynikach klasyfikacji dla metod FAVOR+ i SDPA po usunięciu prefiksu (\texttt{more on}) i skróceniu sekwencji do $2000$ tokenów (N=$50$)}
\smallskip
\label{tab:hyper-predict-change}
\begin{tabular}{lccc}
\hline
\textbf{Metoda} & \textbf{Liczba zmian} & \textbf{$1 \rightarrow 0$} & \textbf{$0 \rightarrow 1$} \\
\hline
FAVOR+ & 8/50 & 7 & 1 \\
SDPA   & 5/50 & 5 & 0 \\
\hline
...
\paragraph{Usunięcie prefiksu i skrócenie do długości $2000$}
Przeanalizowano dokumenty o maksymalnej długości ($4096$ tokenów), które rozpoczynają się od prefiksu \texttt{more on}. W zbiorze testowym występuje $335$ takich przypadków, z tej puli wylosowano $50$ tekstów, a następnie usunięto z nich prefiks orazskrócono tekst do $2000$ tokenów. Następnie sprawdzono, w ilu przypadkach modyfikacja ta zmieniła predykcję modeli względem wersji oryginalnej. Wyniki przedstawione w tab.~\ref{tab:hyper-predict-change} częściowo potwierdzają powyższe rozważania.





\subsection{Hipoteza dotycząca mechanizmu błędów}
Powyższe obserwacje sugerują, że FAVOR+ może mieć trudności z:
\begin{enumerate}
    \item \textbf{Utrzymaniem kontekstu semantycznego w długich sekwencjach} -- aproksymacja softmax może osłabiać zdolność modelu do powiązania informacji z odległych fragmentów tekstu. W długich dokumentach (>2000 tokenów), gdzie kluczowe dla klasyfikacji informacje mogą być rozproszone, model może polegać bardziej na powierzchniowych wskaźnikach (np. obecność określonych słów) niż na głębszym zrozumieniu semantyki całego tekstu.
    
    \item \textbf{Rozróżnianiem sygnału semantycznego od artefaktów formatowania} -- wzorce nieobecne w treningu (np. \texttt{more on:}) mogą być błędnie interpretowane jako oznaka tendencyjności.
    
    \item \textbf{Radzeniem sobie z przesunięciem dystrybucji} -- różnice między zbiorami treningowym i testowym (długość tekstów, wzorce leksykalne) mogą bardziej negatywnie wpływać na FAVOR+ niż na SDPA, ze względu na ograniczenia aproksymacji.
\end{enumerate}

\subsection{Ograniczenia analizy}

Przeprowadzona analiza ma charakter wstępny i eksploracyjny:

\begin{itemize}
    \item \textbf{Mała próba} -- analiza opiera się na zaledwie 20 wyselekcjonowanych przykładach.
    
    \item \textbf{Powierzchowna analiza} -- obserwacje opierają się na prostych miarach (długość tekstu, obecność wzorców prefiksowych) bez głębszej analizy semantycznej czy lingwistycznej. Nie zbadano szczegółowo treści tekstów.
    
    \item \textbf{Charakter selekcji} -- wybrano wyłącznie przypadki rozbieżności między modelami, co nie odzwierciedla typowego zachowania FAVOR+ na całym zbiorze.
    
    \item \textbf{Brak analizy przyczynowej} -- zaobserwowane korelacje nie oznaczają związku przyczynowego. Długość i wzorce prefiksowe mogą współwystępować z innymi, niezbadanymi czynnikami faktycznie odpowiedzialnymi za błędy klasyfikacji.
\end{itemize}

Celem tej analizy było jedynie wstępne rozpoznanie możliwych kierunków dalszych badań, nie dostarczenie definitywnych wyjaśnień mechanizmów błędów FAVOR+. Pełniejsza typologia błędów wymagałaby analizy znacznie większej próby przykładów oraz bardziej szczegółowych metod badawczych.
...
Architektura FAVOR+ umożliwia zamianę mechanizmu uwagi bez konieczności ponownego pretreningu, ponieważ projekcje $Q$, $K$, $V$ pozostają zgodne ze standardową uwagą. Zbadano podejście hybrydowe: wykorzystanie zapisanego stanu modelu SDPA z~pretreningu na Wikipedii, a~następnie zamiana mechanizmu na FAVOR+ ($N_f{=}0.25$) wyłącznie dla etapów TAPT i~dostrajania na zbiorze ArXiv. Wyniki przedstawiono w~tab.~\ref{tab:hybrid-experiment}.
...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Konfiguracja} & \textbf{Czas pretrening [min]} & \textbf{Czas TAPT [min]} & \textbf{Czas dostrajania [min]} & \textbf{F1 [\%]} \\
\midrule
...
Dla elementów $j$ znajdujących się w~tym samym (lub sąsiednim) bloku, lecz w~innym koszyku co token $i$ rozważmy dwa scenariusze:
(1) Jeśli $j$ nie jest podobny do $i$ ($q_i \cdot k_j$ jest małe), to jego wkład do Softmax jest niewielki,  włączenie takich elementów można traktować jako formę regularizacji, ograniczając nadmierne ,,skupienie'' uwagi wyłącznie na~elementach z~własnego koszyka.
(2) Jeśli natomiast $j$ jest podobny do~$i$ ($q_i \cdot k_j$ jest duże), ale trafił do~innego koszyka, to usunięcie maski pozwala nadal uwzględnić go w~uwadze, co redukuje negatywne skutki błędów haszowania.



\begin{figure}[t!]
      \centering
      \includegraphics[width=0.8\textwidth]{static/lsh_maska_vs_brak_maski.pdf} 
...
Wymóg kosztowy (Performer) & Częściowo spełnione & Zgodnie z tab.~\ref{tab:attention-f1-vs-baseline},~\ref{tab:attention-f1-vs-sdpa},~\ref{tab:time-comparison} i~\ref{tab:vram-comparison}\\
Wymóg kosztowy (Reformer) & Częściowo spełnione & Zgodnie z tab.~\ref{tab:attention-f1-vs-baseline},~\ref{tab:attention-f1-vs-sdpa},~\ref{tab:time-comparison} i~\ref{tab:vram-comparison}\\
...
\midrule
Jakość (SDPA) & Spełnione & Zgodnie z tab.~\ref{tab:attention-f1-vs-baseline},~\ref{tab:attention-f1-vs-sdpa}\\
Jakość (Performer) & Częściowo spełnione & Zgodnie z tab.~\ref{tab:attention-f1-vs-baseline},~\ref{tab:attention-f1-vs-sdpa}\\
Jakość (Reformer) & Spełnione & Zgodnie z tab.~\ref{tab:attention-f1-vs-baseline},~\ref{tab:attention-f1-vs-sdpa}\\
...
Zgodność z PEP-8 & Spełnione & Kod źródłowy w załączniku~\ref{app:code} \\
...
Ze względu na koszty obliczeniowe pretrening naszego modelu był znacznie krótszy niż w oryginalnej pracy o BERT~\cite{devlin2019bert}. Podczas pretreningu na Wikipedii model przetworzył około $1.3$~mld tokenów ($10$~epok $\times$ ($450\text{k} \times 128 + 150\text{k} \times 512$)), podczas gdy w oryginalnym BERT było to około $42.6$~mld tokenów. Co więcej, nasz korpus tekstowy obejmował zaledwie $600$~tys. artykułów Wikipedii, czyli  około $9\%$ całego korpusu ($6.4$~mln artykułów). Dodatkowo w BERT wykorzystano nie tylko Wikipedię, ale także BooksCorpus. W rezultacie nasze modele nie pokazują pełnego potencjału, jaki można uzyskać dzięki dłuższemu pretreningowi na większym i bardziej zróżnicowanym zbiorze danych, a następnie przenieść na zadania docelowe.
Z powodu kosztów obliczeniowych, każdą konfigurację trenowano jednokrotnie, co oznacza, że różnice rzędu dziesiątych części punktu procentowego mogą wynikać z~losowości procesu uczenia. Zbadano tylko trzy długości sekwencji ($512$, $4096$, $16384$ tokenów). Eksperymenty przeprowadzono wyłącznie na architekturze $\text{BERT}_{\text{SMALL}}$ -- zachowanie mechanizmów uwagi może różnić się dla większych modeli. Zbadano trzy zbiory danych o~zróżnicowanej charakterystyce, jednak wyniki mogą nie generalizować się na inne zbiory danych oraz inne zadania NLP. Dodatkowo, badane mechanizmy uwagi przybliżonej nie były w żaden sposób niskopoziomowo optymalizowane, w przeciwieństwie do natywnej implementacji SDPA w PyTorch. Ponadto, proces optymalizacji hiperparametrów dostrajania modeli był prowadzony z wykorzystaniem mechanizmu SDPA, co potencjalnie mogło faworyzować go względem mechanizmów FAVOR+ i LSH.



\section{Kierunki dalszych prac}

\subsection{Rozszerzenie zbioru mechanizmów uwagi systemu}
...
\subsection{Walidacja statystyczna} 
...
\subsection{Analiza interpretowalności}
Ponieważ rozważane mechanizmy różnią się sposobem dystrybucji uwagi, wartościowe byłoby porównanie wzorców uwagi pomiędzy modelami -- wizualizacje macierzy uwagi dla wybranych przykładów, analiza koncentracji na tokenach specjalnych oraz preferencji lokalnych vs dalekozasięgowych.

Dodatkowej analizy wymaga mechanizm FAVOR+, którego wyniki klasyfikacji okazały się słabsze, zwłaszcza na zbiorze Hyperpartisan. Co więcej, na Hyperpartisan wyniki wszystkich modeli cechowała duża wariancja, dlatego warto dokładniej sprawdzić możliwe przyczyny -- np. czy modele (lub mechanizmy uwagi) mają tendencję do zapamiętywania słów kluczowych zamiast uczenia się ogólnych reprezentacji języka.

\subsection{Porównanie z gotowymi modelami}
...
% \chapter*{Spis załączników}\label{sec:annex}
% \begin{enumerate}
...
\begin{enumerate}
    \item Załącznik \ref{app:hyperparams}: Szczegółowy opis parametrów
    \item Załącznik \ref{app:baseline-results}: Szczegółowe wyniki klasyfikatora
    \item Załącznik \ref{app:learning-stability}: Stabilność uczenia
    \item Załącznik \ref{app:code}: Kod źródłowy aplikacji
\end{enumerate}
...
\section{Szczegółowy opis parametrów}\label{app:hyperparams}
...
\setcounter{table}{0}
\section{Stabilność uczenia}\label{app:learning-stability}
\begin{table}[h!]
\centering
...
\FloatBarrier
\setcounter{table}{0}
\section{Analiza jakościowa błędów Hyperpartisan}\label{app:qualitative-analysis}
\setlength{\LTcapwidth}{\textwidth}
\begin{longtable}{@{}rcp{0.75\textwidth}@{}}
\caption{Przykłady tekstów ze zbioru Hyperpartisan błędnie sklasyfikowanych przez FAVOR+ jako \textit{partisan}, a poprawnie rozpoznanych przez SDPA jako \textit{neutral}.}
\label{tab:favor-fp-sdpa-correct} \\
\toprule
\textbf{ID} &
\textbf{Długość} &
\textbf{Fragment tekstu} \\
\midrule
\endfirsthead
...
\midrule
\multicolumn{3}{r}{\textit{Kontynuacja na następnej stronie}} \\
\endfoot

\bottomrule
\endlastfoot

6 & 2271 & more on : on september 21st the fed announced that it would be selling \$ 400 billion in short - term treasurys and buying \$ 400 billion in longer - term treasurys to replace them - a maneuver titled "... \\
7 & 1873 & perusing the coverage of president donald j. trump ' s first ten months in office, a reader could easily conclude that the global trading system, which has been in existence essentially since 1945, is... \\
12 & 3044 & for more on : the debate over skyscrapers and their place in the american city has endured for over a century, and new york city has often led the conversation. in 1913, the equitable life assurance s... \\
17 & 4045 & " [ gcmaf ] is also believed, " the web site also reported, " to be capable of treating and reversing autism, hiv, liver / kidney disease and diabetes. " rumor has it that gcmaf has the potential to b... \\
27 & 4096 & much has been said over the past few years about the novelty of the security challenges now facing the united states. in what is still the most popular version of events, history started on 9 / 11, wh... \\
40 & 2088 & with the announcement last week that vice president of communication natalie kerris is leaving twitter after only six months on the job, the company ' s public relations troubles are in the news again... \\
47 & 2365 & more on : this is a guest post by mrs. joan sikand, esq. she has served as development coordinator of the wildlife foundation since its inception in 2000. she has been a member of friends of nairobi n... \\
48 & 3360 & this post is co - authored by sungtae " jacky " park, research associate for korea studies at the council on foreign relations. \& \# 160 ; a version of this post is more on : north korea ' s intense se... \\
51 & 4096 & christine cumming of the new york fed joins andres rozental from the mexican council on foreign relations to discuss current economic trends and the role of the fed in the financial regulatory system.... \\
53 & 4096 & the concept of " american exceptionalism " has been applied mainly to the political differences that separate the united states from the " old world " : the striking absence of any serious american so... \\
55 & 1811 & japan ' s worsening crisis at the fukushima nuclear plant following the march 11 earthquake and tsunami has reignited the debate about the safety of nuclear power. what has occurred at the fukushima n... \\
74 & 3027 & more on : after this week ' s shangri - la dialogue in singapore, which featured the u. s. - china war of words that has come to characterize the security meeting, secretary of defense ashton carter t... \\
75 & 3925 & more on : since 2000, the percent of state budgets devoted to higher education has fallen from 13 percent to 9. 5 percent. this decline in funding has strained public university finances, reduced pay ... \\
81 & 3065 & more on : over the weekend, thailand ' s junta leader, prayuth chan - ocha, announced a firmer roadmap toward civilian rule than the army had previously revealed. prayuth went on thai television and a... \\
90 & 1877 & in a day of stunning developments that revived the ruling effectively shaved last night, a leon county judge ruled that the recounts would begin as early as 8 a. m. today and must be finished by 2 p. ... \\
94 & 1376 & more on : in conflict zones, some families view child marriage as a way to preserve their daughter ' s honor and protect her from the sexual assault and gender - based violence that commonly occurs in... \\
97 & 1538 & more on : the credit risk of oil exporting countries such as venezuela and russia tends to move with the price of oil. as a country ' s oil export revenue improves, so does its ability to pay its debt... \\
103 & 1958 & [ this is the 3rd in the series religion and science : a beautiful friendship ] it is not instruction but provocation that i can receive from another. - ralph waldo emerson upon detecting hypocrisy in... \\
105 & 1868 & more on : there is a the sudanese yellow fever epidemic and the chinese response to it is a reminder of africa ' s disease burden. hiv / aids gets the most attention in the united states, and most of ... \\
130 & 4011 & a quick follow up on one objection to highlighting thailand in the coming foreign exchange report is that the baht has appreciated this year, and, well, to be a really bad actor, you have to be interv... \\
\end{longtable}

\FloatBarrier

\section{Kod źródłowy}\label{app:code}
