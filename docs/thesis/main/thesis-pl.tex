...
W~niniejszej sekcji przedstawiono zbiorczą analizę wyników ze wszystkich etapów eksperymentu. Tabele~\ref{tab:vram-comparison} i~\ref{tab:time-comparison} zestawiają względne różnice w~zużyciu pamięci VRAM oraz czasie treningu dla mechanizmów przybliżonych względem SDPA. Różnice procentowe w~czasie treningu obliczono na podstawie całkowitego czasu obu etapów (suma: liczba epok TAPT $\times$ czas epoki TAPT + liczba epok dostrajania $\times$ czas epoki dostrajania), a~nie pojedynczych epok. Tabela~\ref{tab:attention-f1-comparison} podsumowuje wyniki klasyfikacji F1-macro w~odniesieniu do baseline'u oraz SDPA.
