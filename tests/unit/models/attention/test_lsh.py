...
def test_lsh_attention_shape_and_dtype():
    ...
    attn, x, padding_mask = _random_attention(mask_within_chunks=False)
...
def test_lsh_attention_respects_padding_mask():
    ...
    attn, x, padding_mask = _random_attention(chunk_size=4, mask_within_chunks=True)
    attn.eval()
    out = attn(x, padding_mask=padding_mask)
...
def test_lsh_attention_backward_pass_runs():
    ...
    attn, x, padding_mask = _random_attention(chunk_size=16, mask_within_chunks=False)
...
def test_lsh_attention_multiple_hashes_stable():
    ...
    attn, x, padding_mask = _random_attention(num_hashes=4, mask_within_chunks=False,)
    attn.train()
    out = attn(x, padding_mask=padding_mask)

    assert torch.isfinite(out).all()
    attn, x, padding_mask = _random_attention(num_hashes=4, mask_within_chunks=True,)
    attn.eval()
    out_eval = attn(x, padding_mask=padding_mask)
...
def test_lsh_attention_outputs_are_finite(mask_within_chunks, training, chunk_size):
    ...
    for seed in range(3):
        ...
        attn, x, padding_mask = _random_attention(chunk_size=chunk_size, mask_within_chunks=mask_within_chunks)
        attn.train(training)
        out = attn(
            x,
